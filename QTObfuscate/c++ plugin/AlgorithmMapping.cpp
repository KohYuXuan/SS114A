...
bool AlgorithmMapping::algorithmMap(string& s_input, string& s_sourceOutput, string& s_headerOutput,
...
bool AlgorithmMapping::algorithmDefine(string& s_input, string& s_sourceOutput, string& s_headerOutput,
                                vector<string>& v_listOfHeaderFiles, bool& b_addHeaderFile)
{
...
void AlgorithmMapping::modifyString(string& s_input, string& s_randOutput, string& s_output)
{
    int i_num;
...
void AlgorithmMapping::mapString(string& s_input)
{
	int i_num;
	string s_name, s_name2, s_argument, s_randName, s_headerFile;
	vector<int> v_charPos;
    ...
    // do not randomise C++ keywords
    if (m_listOfKeywords.find(s_input) != m_listOfKeywords.end() ) {
        m_listOfBlocks.push_back(s_input);
        return;
    }
		if (isalpha(s_input[0]) ) {
            string s_rand, s_block ;
            i_num = NUM_NAME;
            s_randName = RAND_NAME;
			s_rand = nameGenerator(s_randName, i_num);
			s_input = rot13(s_input);
            m_mappingOutput.insert( pair<string,string>(s_input, s_rand));
            m_iter = m_mappingOutput.find(s_input);
            s_block = m_iter->second;

            // return existing or newly inserted random names from std::set
			m_listOfBlocks.push_back(s_block );
            s_input.clear();
			return;
        }
...
void AlgorithmMapping::skipQuote(const string& s_input, int& i_skip, char c_quote)
{
...
void AlgorithmMapping::skipHeader(const string& s_input, int& i_skip, char c_bracket)
{
	int i = i_skip;
...
void AlgorithmMapping::randomiseString(string& s_inOutput, vector<int>& v_charPos)
{
...
string AlgorithmMapping::nameGenerator(string& s_name, int i_num)
{
	string s_output;
	int i_randNum=0;
...
void AlgorithmMapping::format(vector<string>& v_input, string& s_output)
{
    int i_line = 0;
...
void AlgorithmMapping::convertToString(vector<string>& v_input, string& s_output)
{
    for (int j=0; j<v_input.size(); ++j) {
            s_output += v_input[j];
    }
...
string AlgorithmMapping::addSpace(int i_numSpaces)
{
	string s_output;
...
std::string AlgorithmMapping::rand_alnum_str (std::string::size_type sz)
{
    std::string s;
    s.reserve  (sz);
...
void AlgorithmMapping::removeComment(string& s_input, int& i_skip)
{
...
void AlgorithmMapping::setListOfKeywords(set<string>& set_listOfKeywords)
{
    m_listOfKeywords = set_listOfKeywords;
}


void AlgorithmMapping::clearAlgorithmBuffer()
{
    m_listOfDefine.clear();
    m_listOfInclude.clear();
    m_listOfBlocks.clear();
...
void AlgorithmMapping::getMappingList(map<string, string>& m_output)
{
    m_output.insert( m_mappingOutput.begin(), m_mappingOutput.end() );
}

string AlgorithmMapping::rot13(string s_input)
{
